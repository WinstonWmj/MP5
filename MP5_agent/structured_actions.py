            ...
            if (movedir == 0):
                ...
                while (events['location_stats']['pos'][0]>math.floor(events['location_stats']['pos'][0])-0.45):
                    if (try_num > 20):
                        return
                    try_num += 1
                    events,_,_,_ = env.step([0,2,1,12,12,0,0,0]); save_rgb_for_video(events)
                    # events,_,_,_ = env.step([0,0,0,12,12,0,0,0]); save_rgb_for_video(events)
            elif (movedir == 1):# left
                try_num = 0
                while (events['location_stats']['pos'][2]<math.ceil(events['location_stats']['pos'][2])+0.45):
                    if (try_num > 20):
                        return
                    try_num += 1
                    events,_,_,_ = env.step([0,1,1,12,12,0,0,0]); save_rgb_for_video(events)
                    # events,_,_,_ = env.step([0,0,0,12,12,0,0,0]); save_rgb_for_video(events)
            else:
                try_num = 0
                while (events['location_stats']['pos'][2]>math.floor(events['location_stats']['pos'][0])-0.45):
                    if (try_num > 20):
                        return
                    try_num += 1
                    events,_,_,_ = env.step([2,0,1,12,12,0,0,0]); save_rgb_for_video(events)  
                    # events,_,_,_ = env.step([0,0,0,12,12,0,0,0]); save_rgb_for_video(events)
